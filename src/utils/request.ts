...
const handleData = async ({ data, status, statusText }: any) => {
  ...
  if (codeVerificationArray.indexOf(data[statusName]) + 1)
    switch (code) {
      case 200:
      ...
      // break
      case 201:
        // { code: 201, msg: 'ok', data: { list, ...{ total: mockList.length } }
        // { code: 201, msg: 'ok', data: [] }
        //console.log(data)
        if (data.data != null) {
          if (data.data.list != null) {
            data.data.list = toTree(data.data.list)
          } else {
            if (data.data.length != undefined) {
              //是数组
              data.data = toTree(data.data)
            } else {
              // 不是数组返回为Object需转为list
              data.data = [data.data]
            }
          }
        } else {
...
//每次想携带cookie的axios请求,加上withCredentials:true的属性
...
instance.defaults.timeout = 30000 // 设置超时时间为30秒钟
