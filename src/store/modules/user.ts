...
import { getUserInfo, login, logout, checkLogCount } from '~/src/api/login'
...
export const useUserStore = defineStore('user', {
  ...
  actions: {
     ...
     */
    async checklogcount() {
      const { data: msg } = await checkLogCount()
      if (msg) {
        gp.$baseMessage(msg, 'error', 'vab-hey-message-error')
      }
    },
    /**
    ...
    async getUserInfo() {
      const {
        data: { userId, username, avatar, roles, permissions },
      } = await getUserInfo()
      /**
