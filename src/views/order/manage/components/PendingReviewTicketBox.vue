    ...
    <vab-card shadow="hover">
      <vab-query-form>
        <vab-query-form-top-panel :span="12">
          ...
          <el-button plain type="primary" @click="handleWithdraw()">
...
<script>
  import { getPendingReviewTicketList } from '@/api/order/orderTicketManage'
  import { WithdrawTicket } from '@/api/order/orderTicketManage'
  import {
    Plus,
    // Upload,
    // Edit,
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const fetchData = async () => {
        state.listLoading = true
        const { data } = await getPendingReviewTicketList(state.searchForm)
      ...
      const handleWithdraw = async (row) => {
        ...
        $baseConfirm('你确定要撤回该工作票吗？', null, async () => {
          const result = await WithdrawTicket(state.editRow)
