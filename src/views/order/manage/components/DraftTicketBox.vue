    ...
    <vab-card shadow="hover">
      <vab-query-form>
        <vab-query-form-top-panel :span="12">
          ...
          <el-button plain type="primary" @click="handleDelete()">
      ...
      >
        ...
        >
          <template #default="{ row }">
            {{
              // 状态  0-草稿，1-已撤销，2-已打回
              row.status == 0 ? '草稿' : row.status == 1 ? '已撤销' : '已打回'
            }}
          </template>
        </el-table-column>
        <el-table-column
...
<script>
  import { getDraftTicketList } from '@/api/order/orderTicketManage'
  import { doDelete } from '@/api/order/orderTicketManage'
  import {
    Plus,
    // Upload,
    Edit,
  ...
  export default defineComponent({
    ...
    components: {
      ...
      Edit: defineAsyncComponent(() =>
        import('@/views/order/manage/components/DraftTicketEdit.vue')
      ),
    },
    setup() {
      ...
      const fetchData = async () => {
        state.listLoading = true
        const { data } = await getDraftTicketList(state.searchForm)
