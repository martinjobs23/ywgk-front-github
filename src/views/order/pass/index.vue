...
<script>
  import { getOrderList } from '@/api/order/orderPass'
  import { passOrder, refuseOrder } from '@/api/order/orderPass'
  import {
    Plus,
    Delete,
    DArrowLeft,
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const passOrderClick = async (row) => {
        ...
        state.editRow = row
        state.editRow.status = '4'
        $baseConfirm('你确定要审核通过该工作票吗？', null, async () => {
          const result = await passOrder(state.editRow)
