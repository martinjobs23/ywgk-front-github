<template>
  <div class="authPolicy-management-container">
    <vab-card shadow="hover">
      <vab-query-form>
        <vab-query-form-top-panel :span="12">
          ...
          <!-- <el-button :icon="Edit" plain type="primary" @click="handleSet()">
            全局策略
          </el-button> -->
    ...
    <!-- <setting ref="settingRef" @edit-finish="onEditFinish" /> -->
  </div>
</template>

<script>
  import { doDelete, getList, getSettingList } from '@/api/admin/authPolicy'
  import {
    Plus,
    Edit,
    Delete,
  ...
  export default defineComponent({
    name: 'AuthPolicy',
    components: {
      Edit: defineAsyncComponent(() => import('./components/AuthPolicyEdit')),
      // Setting: defineAsyncComponent(() =>
      //   import('./components/GlobalPolicyEdit')
      // ),
    },
    setup() {
      ...
      const handleDelete = (row) => {
        if (row && row.id) {
          $baseConfirm('你确定要删除当前项吗', null, async () => {
            const { data } = await doDelete({ id: row.id, name: row.name })
            if (data === 0)
          ...
          if (selectedRows.length > 0) {
            ...
            $baseConfirm('你确定要删除选中项吗', null, async () => {
              const { data } = await doDelete({ id: ids, name: names })
              if (data === 0)
                $baseMessage('删除失败', 'error', 'vab-hey-message-error')
              else {
                $baseMessage('删除成功', 'success', 'vab-hey-message-success')
      ...
      // const handleSet = () => {
      //   state['settingRef'].showEdit(state.settinglist)
      // }
