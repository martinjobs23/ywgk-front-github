    ...
    <vab-card shadow="hover">
      ...
      >
        ...
        <el-table-column
          label="认证策略"
          min-width="200"
          prop="auth_policy_name"
          show-overflow-tooltip
          sortable
        >
          <template #default="{ row }">
            {{ row.auth_policy_name }}
          </template>
        </el-table-column>
...
<script>
  import { doDelete, getTree, getAuthPolicy } from '@/api/admin/role'
  import {
    Plus,
    Edit,
    Delete,
  ...
  export default defineComponent({
    name: 'Role',
    components: {
      Edit: defineAsyncComponent(() => import('./components/RoleEdit')),
    },
    setup() {
      ...
      const handleAdd = () => {
        ...
        state.editRow = null
        state['editRef'].showAdd(state.policyData, state.list, pid)
      }
      const handleEdit = (row) => {
        ...
        state.editRow = row
        state['editRef'].showEdit(state.policyData, state.list, row)
      }
      ...
      const setPolicyName = (list) => {
        if (!state.policyData) return
        list.forEach((x) => {
          state.policyData.forEach((p) => {
            if (x.auth_policy_id == p.id) {
              x.auth_policy_name = p.name
            }
          })
          if (x.children && x.children.length > 0) setPolicyName(x.children)
        })
      }
