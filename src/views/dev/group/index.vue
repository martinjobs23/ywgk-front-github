...
<script>
  import { doDelete, getTree, setSort } from '@/api/dev/group'
  ...
  export default defineComponent({
    name: 'Group',
    components: {
      Edit: defineAsyncComponent(() => import('./components/GroupEdit')),
    },
    setup() {
      ...
      const handleAdd = () => {
        ...
        state.editRow = null
        state['editRef'].showAdd(state.userData, state.list, pid)
      }
      const handleEdit = (row) => {
        ...
        state.editRow = row
        state['editRef'].showEdit(state.userData, state.list, row)
      }
