        ...
        <el-form enctype="multipart/form-data" inline :model="uploadForm">
          <el-form-item>
            <el-upload
              ref="uploadRef"
              accept=".xlsx, .xls"
              action="http://localhost:8080/summer/dialogFormVisible/info/import"
              :auto-upload="false"
...
<script>
  import { doImport } from '@/api/dev/info'
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const showUpload = (
        ...
        state.groupNameMap = treeToNameMap(groupData)
            ...
            outdata.forEach((excel_row) => {
              ...
              state.fieldSet.forEach((f) => {
                if (excel_row[f.label]) {
                  ...
                  } else if (f.label.indexOf('设备组') > -1) {
                    //根据分组名称 获取 group_id
                    const group_id = state.groupNameMap.get(row[f.prop])
                    if (group_id) row.group_id = group_id
                    else row.group_id = 0 //未分组
                  }
                  excle_col_count++
                }
              })
