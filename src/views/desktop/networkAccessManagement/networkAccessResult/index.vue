      ...
      >
        ...
        >
          <template #default="{ row }">
            {{
              row.username.match(/username(.+?)username/)
                ? row.username.match(/username(.+?)username/)[1]
                : '未知用户'
            }}
          </template>
        </el-table-column>
        ...
        >
          <template #default="{ row }">
            {{ row.username.substring(0, 32) }}
          </template>
        </el-table-column>
        <el-table-column label="设备MAC" show-overflow-tooltip sortable>
          <template #default="{ row }">
            {{ row.username.substring(32, 44) }}
          </template>
        </el-table-column>
        <!--        <el-table-column-->
        <!--          label="设备IP"-->
        ...
        >
          <template #default="{ row }">
            <vab-icon
              v-if="row.reply == 'Access-Accept'"
              icon="checkbox-circle-fill"
              style="color: #92ca41"
            />
            <vab-icon
              v-if="row.reply == 'Access-Reject'"
        ...
        <el-table-column align="center" label="操作" width="80">
          <template #default="{ row }">
            <el-button type="text" @click="handlePassChange(row)">
              修改密码
            </el-button>
          </template>
        </el-table-column>
...
<script>
  import { getUserNetAccessList } from '@/api/desktop/networkAccessManagement'
  import {
    Plus,
    Edit,
    Delete,
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const fetchData = async () => {
        ...
        console.log(state)
        const { data } = await getUserNetAccessList(state.queryForm)
