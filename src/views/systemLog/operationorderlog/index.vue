...
<script>
  import { getTerminalHardwareAlterList } from '@/api/systemLog/getPage'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handleExport = () => {
        ...
        if (state.list.length) {
          ...
          import('@/utils/excel').then((excel) => {
            const tHeader = ['用户名', '终端IP地址', '描述', '变更时间']
            // const export_type = row.type === 1 ? '安装' : '卸载'
            const filterVal = ['dev_name', 'dev_ip', 'description', 'time']
            const list = state.list
            const data = formatJson(filterVal, list)
            state.title = '终端硬件变更日志'
      ...
      const fetchLogData = async () => {
        state.listLoading = true
        const { data } = await getTerminalHardwareAlterList(state.queryForm)
