  ...
  <div class="software-Install-Uninstall">
    <vab-card shadow="hover">
      ...
      >
        ...
        <el-table-column label="指令标题" prop="title" show-overflow-tooltip />
        <el-table-column label="终端IP地址" prop="ip" show-overflow-tooltip />
        <el-table-column
          label="指令内容"
          prop="content"
...
<script>
  import { getInstructionList } from '@/api/systemLog/getPage'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handleExport = () => {
        ...
        if (state.list.length) {
          ...
          import('@/utils/excel').then((excel) => {
            const tHeader = ['指令标题', '终端IP地址', '指令内容', '下发时间']
            // const export_type = row.type === 1 ? '安装' : '卸载'
            const filterVal = ['title', 'ip', 'content', 'time']
      ...
      const fetchData = async () => {
        state.listLoading = true
        const { data } = await getInstructionList(state.queryForm)
