...
<script>
  import { getSoftwarelibraryList } from '@/api/systemLog/getPage'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handleExport = () => {
        ...
        if (state.list.length) {
          ...
          import('@/utils/excel').then((excel) => {
            const tHeader = ['软件名称', '软件HASH', '操作类型', '操作时间']
      ...
      const fetchData = async () => {
        state.listLoading = true
        const { data } = await getSoftwarelibraryList(state.queryForm)
