...
<script>
  ...
  import { doDelete, getPage } from '@/api/ipam/alloc'
  import {
    Plus,
    Edit,
    Delete,
  ...
  export default defineComponent({
    name: 'Alloc',
    components: {
      Edit: defineAsyncComponent(() => import('./components/AllocEdit')),
    },
    setup() {
      ...
      const handleBind = (row) => {
        ...
        if (row && row.id) {
          ...
          selectedRows.forEach((row) => {
            if (row.ip && row.mac != null && row.mac.length != 0)
              ids.push(row.id)
          })
        }
        ...
        if (ids.length == 0) {
          $baseMessage(
            '未选中任何行，或没有有效的IP和MAC地址',
            'error',
            'vab-hey-message-error'
          )
        } else
          $baseMessage(
            '自动配置交换机端口进行IP和MAC地址绑定。。。',
            'success',
