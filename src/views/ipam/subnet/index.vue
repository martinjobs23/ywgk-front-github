<template>
  <div class="subnet-management-container">
    <vab-card shadow="hover">
      ...
      >
        ...
        <el-table-column
          ...
          sortable
        />
        <el-table-column align="center" label="固定IP" width="80">
          <template #default="{ row }">
            {{ row.fixed == 1 ? '是' : '' }}
          </template>
        </el-table-column>
        <el-table-column align="center" label="保留" width="80">
          <template #default="{ row }">
            {{ row.is_reserved == 1 ? '是' : '' }}
          </template>
        </el-table-column>
        <el-table-column align="center" label="扫描" width="80">
          <template #default="{ row }">
            {{ row.noscan == 0 ? '是' : '' }}
          </template>
        </el-table-column>
...
<script>
  import { doDelete, getTree, setSort } from '@/api/ipam/subnet'
  ...
  export default defineComponent({
    name: 'Subnet',
    components: {
      Edit: defineAsyncComponent(() => import('./components/SubnetEdit')),
    },
    setup() {
      ...
      const handleAdd = () => {
        ...
        state.editRow = null
        state['editRef'].showAdd(state.treeData, state.list, pid)
      }
      const handleEdit = (row) => {
        ...
        state.editRow = row
        state['editRef'].showEdit(state.treeData, state.list, row)
      }
      ...
      function setOrgName(rootList, orgMap) {
        rootList.forEach((item) => {
          if (item.org_id != 0) item.org_name = orgMap.get(item.org_id)
          //console.log(item)
          if (item.children && item.children.length > 0)
            setOrgName(item.children, orgMap)
        })
      }
