...
<script>
  import { operatingSystem } from '@/api/analysis/analysis'
  import {
    Plus,
    Edit,
    Delete,
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const fetchData = async () => {
        ...
        const { data } = await operatingSystem(state.queryForm)

        const data_total = await operatingSystem()
        state.list = data.jsonData
        state.list_total = data_total.data.jsonData
        console.log('++++++++++++++++++++++++++++++++++')
      ...
      const handleExport = () => {
        ...
        if (state.list.length) {
          ...
          import('@/utils/excel').then((excel) => {
            const tHeader = ['操作系统类型', '设备数量']
            const filterVal = ['os', 'count']
            const list = state.list_total
            const data = formatJson(filterVal, list)
            state.title = '操作系统类型统计'
