...
<script>
  import { terminalAlert } from '@/api/analysis/analysis'
  import {
    Plus,
    Edit,
    Delete,
  ...
  // import { columnStyle } from '~/src/config'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const fetchData = async () => {
        ...
        const { data } = await terminalAlert(state.queryForm)
      ...
      const handleExport = () => {
        ...
        if (state.list.length) {
          ...
          import('@/utils/excel').then((excel) => {
            const tHeader = ['设备名称', '异常类型', '处理措施', '异常数量']
            const filterVal = ['dev_name', 'alertType', 'type', 'count']
            const list = state.list
            const data = formatJson(filterVal, list)
            state.title = '终端告警分析统计'
