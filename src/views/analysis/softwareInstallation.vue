...
<script>
  import {
    softwareInstallation_device,
    softwareInstallation_software,
    ...
    setup() {
      ...
      const fetchData = async () => {
        ...
        if (state.device_software) {
          const { data } = await softwareInstallation_device(state.queryForm)
          state.list = data.jsonData
          console.log(data)
          console.log(data.jsonData)
          state.total = data.totalCount
        } else {
          const { data } = await softwareInstallation_software(state.queryForm)
          state.list = data.jsonData
          console.log(data)
          console.log(data.jsonData)
          state.total = data.totalCount
        ...
        if (state.list.length) {
          ...
          if (state.device_software) {
            import('@/utils/excel').then((excel) => {
              const tHeader = ['设备分组', '软件安装数量']
              const filterVal = ['name', 'count']
              const list = state.list
              const data = formatJson(filterVal, list)
              state.title = '软件安装统计(设备角度)'
            ...
            import('@/utils/excel').then((excel) => {
              const tHeader = ['软件名称', '软件安装数量']
              const filterVal = ['sw_name', 'count']
              const list = state.list
              const data = formatJson(filterVal, list)
              state.title = '软件安装统计(软件角度)'
              excel.export_json_to_excel({
                header: tHeader,
                data,
                filename: state.title,
